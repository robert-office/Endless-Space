...
func _process(delta):
	if global.nave_carregada:
		NAVE_ATUAL = global.nave_player
	else:
		NAVE_ATUAL = null
...
func updateBallPos() -> void:
	...
	# SETA A POSIÇÃO DA BOLINHA E DO POSICIONAMENTO PARA A ROTAÇÃO DA NAVE
	global.ball_pos = ball.global_position;
	global.center_point = global_position
